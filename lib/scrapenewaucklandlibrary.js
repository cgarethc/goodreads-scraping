...
function cleanTitle(title) {
  ...
  // Auckland also has annoying trailing text on titles, remove that as well:
  cleanTitle = cleanTitle.replace(/\s+:\s+a novel$/, '');
  cleanTitle = cleanTitle.replace(/\s+:\s+a memoir$/, '');
  cleanTitle = cleanTitle.replace(/\s+:\s+a biography$/, '');
  return cleanTitle;
}
...
exports.search = async (author, title) => {
  ...
  try {
    ...
    if (response.totalResults === 0) {
      ...
      function addBooksOfType(firstRecord, formatName, typeName, result) {
        ...
        if (formats.length > 0) {
          ...
          for (let url of urls) {
            // only add if the domain is not samples.overdrive.com or excerpts.cdn.overdrive.com
            if (url.indexOf('samples.overdrive.com') < 0 && url.indexOf('excerpts.cdn.overdrive.com') < 0) {
              result.push({
                title: cleanTitle(firstRecord.title),
    ...
    console.error("Request to", url, JSON.stringify(body), "failed", err.message, err.status);
    return [];
  }
};
